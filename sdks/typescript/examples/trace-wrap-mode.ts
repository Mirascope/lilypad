 ...
 * to get a Trace object that allows post-execution operations like assignment and tagging.
 */
...
async function runExamples() {
  ...
  // Example 1: Using synchronous trace with tags
  ...
  console.log('Tags added to trace\n');

  // Example 2: Using asynchronous trace with assignment and tags
  ...
  console.log('Assignments added to async trace\n');
  ...
  // Tag the trace based on report contents
  await reportTrace.tag(
    'monthly-report',
    report.data.every((v) => v > 0) ? 'positive-data' : 'mixed-data',
  );
