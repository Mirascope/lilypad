...
def cleanup_file_descriptors() -> Generator[None, None, None]:
...
@pytest.fixture(autouse=True)
def prevent_recursion() -> Generator[None, None, None]:
...
@pytest.fixture(autouse=True)
def disable_capture_for_problematic_tests(request: pytest.FixtureRequest) -> None:
...
def mock_external_dependencies(request: pytest.FixtureRequest) -> Generator[None, None, None]:
    ...
    skip_subprocess_mock = "closure" in test_name.lower() or "test_closure.py" in test_file or "/closure/" in test_file

    # Create proper mock responses with integer status codes
    def create_mock_httpx_client(*args: Any, **kwargs: Any) -> Mock:
    ...
    def create_mock_async_httpx_client(*args: Any, **kwargs: Any) -> Mock:
    ...
    def create_mock_requests_response() -> Mock:
        mock_response = Mock()
        mock_response.status_code = 200  # Actual integer, not Mock
        mock_response.json.return_value = {"functions": [], "items": []}
        mock_response.raise_for_status.return_value = None
