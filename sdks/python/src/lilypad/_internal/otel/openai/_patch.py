...
from collections.abc import Callable, Awaitable, Iterator
from contextlib import contextmanager

from opentelemetry.trace import Status, Tracer, SpanKind, StatusCode, Span
...
@contextmanager
def _span(
    tracer: Tracer, kwargs: dict[str, Any], instance: OpenAI | AsyncOpenAI
) -> Iterator[Span]:
...
def _sync_wrapper(
    ...
    if not handle_stream:
        ...
        def traced_method_completion(
            ...
            with _span(tracer, kwargs, instance) as span:
...
def _async_wrapper(
    ...
    if not handle_stream:
        ...
        async def traced_method_non_stream(
            wrapped: Callable[P, Awaitable[ChatCompletion]],
            instance: AsyncOpenAI,
            args: tuple[Any, ...],
            kwargs: dict[str, Any],
        ...
        return traced_method_non_stream
    else:
