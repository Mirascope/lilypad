...
_P = ParamSpec("_P")
SyncFunc = Callable[_P, _BaseCallResponseT | _BaseStreamT | _ResponseModelT | _BaseStructuredStreamT]
AsyncFunc = Callable[
    _P,
    Awaitable[_BaseCallResponseT | _BaseStreamT | _ResponseModelT | _BaseStructuredStreamT],
]
_T = TypeVar("_T")
_R = TypeVar("_R")
...
def middleware_factory(
    *,
    custom_context_manager: Callable[[SyncFunc | AsyncFunc], AbstractContextManager[_T]] = default_context_manager,
    custom_decorator: Callable | None = None,
    handle_call_response: Callable[[BaseCallResponse, SyncFunc | AsyncFunc, _T | None], None] | None = None,
    handle_call_response_async: Callable[[BaseCallResponse, SyncFunc | AsyncFunc, _T | None], Awaitable[None]]
    | None = None,
    handle_stream: Callable[[BaseStream, SyncFunc | AsyncFunc, _T | None], None] | None = None,
    handle_stream_async: Callable[[BaseStream, SyncFunc | AsyncFunc, _T | None], Awaitable[None]] | None = None,
    handle_response_model: Callable[[ResponseModel, SyncFunc | AsyncFunc, _T | None], None] | None = None,
    handle_response_model_async: Callable[[ResponseModel, SyncFunc | AsyncFunc, _T | None], Awaitable[None]]
    | None = None,
    handle_structured_stream: Callable[[BaseStructuredStream, SyncFunc | AsyncFunc, _T | None], None] | None = None,
    handle_structured_stream_async: Callable[[BaseStructuredStream, SyncFunc | AsyncFunc, _T | None], Awaitable[None]]
    | None = None,
    handle_error: Callable[[Exception, SyncFunc | AsyncFunc, _T | None], Any] | None = None,
    handle_error_async: Callable[[Exception, SyncFunc | AsyncFunc, _T | None], Awaitable[Any]] | None = None,
    ...
    def decorator(
        ...
        if fn_is_async(fn):
            ...
            async def wrapper_async(*args: _P.args, **kwargs: _P.kwargs) -> _R:
                ...
                if isinstance(result, BaseCallResponse) and handle_call_response_async is not None:
                    ...
                    class MiddlewareAsyncStream(original_class):
                        __aiter__ = custom_decorator(fn)(new_stream_aiter) if custom_decorator else new_stream_aiter

                    result.__class__ = MiddlewareAsyncStream
                elif isinstance(result, ResponseModel) and handle_response_model_async is not None:
                    ...
                    class MiddlewareAsyncStructuredStream(original_class):
                        __aiter__ = custom_decorator(fn)(new_aiter) if custom_decorator else new_aiter
            ...
            return custom_decorator(fn)(wrapper_async) if custom_decorator else wrapper_async
        else:
            ...
            def wrapper(*args: _P.args, **kwargs: _P.kwargs) -> _R:
                ...
                if isinstance(result, BaseCallResponse) and handle_call_response is not None:
                    ...
                    class MiddlewareStream(original_class):  # pyright: ignore [reportGeneralTypeIssues]
                        __iter__ = custom_decorator(fn)(new_stream_iter) if custom_decorator else new_stream_iter
                    ...
                    def new_iter(
                        ...
                        Generator[tuple[Any, Any | None], None, None] | Generator[Any, None, None],
                        Any,
                        None,
                    ]:  # noqa: ANN401
                        # Create the context manager when user iterates over the stream
                    ...
                    class MiddlewareStructuredStream(original_class):  # pyright: ignore [reportGeneralTypeIssues]
                        __iter__ = custom_decorator(fn)(new_iter) if custom_decorator else new_iter
