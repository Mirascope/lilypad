...
class StripeWebhookResponse(BaseModel):
    ...
    __properties: ClassVar[List[str]] = ["status", "event", "message"]

    @field_validator('status')
    def status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['success', 'error', 'ignored']):
            raise ValueError("must be one of enum values ('success', 'error', 'ignored')")
        return value
