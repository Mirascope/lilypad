...
from lilypad._opentelemetry._opentelemetry_anthropic import AnthropicInstrumentor


class TestAnthropicInstrumentor:
    """Test AnthropicInstrumentor class."""
    ...
    def test_instrumentation_dependencies(self):
        ...
        assert "anthropic>=0.29.0,<1.0" in dependencies

    @patch("lilypad._opentelemetry._opentelemetry_anthropic.get_tracer")
    @patch("lilypad._opentelemetry._opentelemetry_anthropic.wrap_function_wrapper")
    @patch("lilypad._opentelemetry._opentelemetry_anthropic.chat_completions_create")
    @patch("lilypad._opentelemetry._opentelemetry_anthropic.chat_completions_create_async")
    def test_instrument_basic(
        ...
        mock_get_tracer.assert_called_once_with(
            "lilypad._opentelemetry._opentelemetry_anthropic",
            "0.1.0",
            None,
        ...
        mock_create_async.assert_called_once_with(mock_tracer)
        ...
        expected_calls = [
            ("anthropic.resources.messages", "Messages.create", mock_create_wrapper),
            ("anthropic.resources.messages", "AsyncMessages.create", mock_create_async_wrapper),
        ]
    ...
    @patch("lilypad._opentelemetry._opentelemetry_anthropic.unwrap")
    def test_uninstrument_basic(self, mock_unwrap):
        """Test basic _uninstrument functionality."""
        instrumentor = AnthropicInstrumentor()

        # Create mock anthropic module structure
        mock_messages = Mock(spec=["create"])
        mock_async_messages = Mock(spec=["create"])
    ...
    @patch(
        "lilypad._opentelemetry._opentelemetry_anthropic.wrap_function_wrapper",
    ...
    @patch("lilypad._opentelemetry._opentelemetry_anthropic.unwrap", side_effect=Exception("Unwrap error"))
    def test_uninstrument_unwrap_error(self, mock_unwrap):
        ...
        # Patch the import to return our mock
        with patch.dict("sys.modules", {"anthropic": mock_anthropic}), pytest.raises(Exception, match="Unwrap error"):
            # Should raise exception when unwrap fails
            instrumentor._uninstrument()

    def test_inheritance(self):
        """Test that AnthropicInstrumentor properly inherits from BaseInstrumentor."""
