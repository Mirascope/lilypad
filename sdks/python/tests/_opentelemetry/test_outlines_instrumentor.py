...
from src.lilypad._opentelemetry._opentelemetry_outlines import OutlinesInstrumentor
import src.lilypad._opentelemetry._opentelemetry_outlines as outlines_module
...
class TestOutlinesInstrumentor:
    ...
    @patch("src.lilypad._opentelemetry._opentelemetry_outlines.get_tracer")
    @patch("src.lilypad._opentelemetry._opentelemetry_outlines.wrap_function_wrapper")
    @patch("src.lilypad._opentelemetry._opentelemetry_outlines.model_generate")
    @patch("src.lilypad._opentelemetry._opentelemetry_outlines.model_generate_stream")
    def test_instrument_basic(
        ...
        outlines_module._patched_targets.clear()
        ...
        mock_get_tracer.assert_called_once_with(
            "src.lilypad._opentelemetry._opentelemetry_outlines",
            "0.1.0",
            None,
    ...
    def test_instrument_tracks_patched_targets(
        ...
        assert len(outlines_module._patched_targets) > 10  # There are many models being instrumented
        ...
        for target in expected_targets:
            assert target in outlines_module._patched_targets
    ...
    def test_instrument_handles_kwargs(
        ...
        assert len(outlines_module._patched_targets) > 10

    def test_patched_targets_is_list(self):
        """Test that outlines_module._patched_targets is initialized as a list."""
        assert isinstance(outlines_module._patched_targets, list)

    def test_patched_targets_can_be_cleared(self):
        """Test that outlines_module._patched_targets can be cleared and modified."""
        # Add some test data
        outlines_module._patched_targets.extend(
            [
        ...
        assert len(outlines_module._patched_targets) >= 2

        # Clear and verify
        outlines_module._patched_targets.clear()
        assert len(outlines_module._patched_targets) == 0

        # Add new data
        outlines_module._patched_targets.append(("new.module", "NewClass", "new_method"))
        assert len(outlines_module._patched_targets) == 1
        assert outlines_module._patched_targets[0] == ("new.module", "NewClass", "new_method")
    ...
    def test_instrument_with_mock_patch_functions(self, mock_wrap_function_wrapper, mock_get_tracer):
        ...
        with (
            patch("src.lilypad._opentelemetry._opentelemetry_outlines.model_generate") as mock_model_generate,
            patch("src.lilypad._opentelemetry._opentelemetry_outlines.model_generate_stream") as mock_model_generate_stream,
        ):
            mock_generate_wrapper = Mock()
            mock_stream_wrapper = Mock()
            mock_model_generate.return_value = mock_generate_wrapper
    ...
    @patch("src.lilypad._opentelemetry._opentelemetry_outlines.unwrap")
    @patch("builtins.__import__", return_value=Mock())
    def test_uninstrument(self, mock_import, mock_unwrap):
        """Test uninstrument restores original methods - covers lines 144-148."""
        # Clear and add test targets
        outlines_module._patched_targets.clear()
        outlines_module._patched_targets.extend([
            ("test.module1", "TestClass1", "method1"),
            ("test.module2", "TestClass2", "method2"),
            ("test.module3", "TestClass3", "method3"),
        ])
        
        instrumentor = OutlinesInstrumentor()
        instrumentor._uninstrument()
        
        # Verify unwrap was called for each target in reverse order
        assert mock_unwrap.call_count == 3
        assert mock_import.call_count == 3
        
        # Verify import calls were made with correct parameters
        import_calls = [call[0] for call in mock_import.call_args_list]
        assert import_calls[0] == ("test.module3", )
        assert import_calls[1] == ("test.module2", )
        assert import_calls[2] == ("test.module1", )
        
        # Verify patched targets was cleared
        assert len(outlines_module._patched_targets) == 0

    @patch("src.lilypad._opentelemetry._opentelemetry_outlines.unwrap")
    @patch("builtins.__import__")
    def test_uninstrument_with_exception(self, mock_import, mock_unwrap):
        """Test uninstrument handles exceptions gracefully."""
        ...
        # Verify patched targets was still cleared
        assert len(outlines_module._patched_targets) == 0
