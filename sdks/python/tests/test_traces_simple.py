...
def test_get_span_uuid_no_match():
...
def test_get_span_uuid_empty_response():
...
def test_get_span_uuid_multiple_spans():
...
@pytest.mark.asyncio
async def test_async_get_span_uuid_success():
...
def test_flush_behavior_in_get_span_uuid():
...
def test_no_duplicate_flush_when_already_flushed():
...
def test_assign_with_empty_email_list():
...
def test_tag_with_empty_tags_returns_early():
...
def test_annotate_with_no_annotations():
