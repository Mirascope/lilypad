...
def test_export_failure(mock_get_settings, mock_get_client):
    ...
    mock_span.status = Mock(status_code=Mock(name="OK"))
    mock_span.events = []
    mock_span.links = []
...
def test_configure_auto_llm(mock_trace, mock_find_spec):
    ...
    with (
        ...
        patch("lilypad._opentelemetry.MistralInstrumentor") as mock_mistral,
...
def test_span_to_dict_minimal(mock_get_settings, mock_get_client):
    ...
    mock_span.end_time = 2000
    mock_span.attributes = None
    mock_span.status = Mock(status_code=Mock(name="UNSET"))
    mock_span.events = []
    mock_span.links = []
