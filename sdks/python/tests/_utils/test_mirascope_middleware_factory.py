...
from lilypad._utils.mirascope_middleware_factory import (
...
async def test_middleware_factory_call_response_async() -> None:
    ...
    decorate = middleware_factory(handle_call_response_async=handle_call_response_async)(async_fn)
...
async def test_middleware_factory_base_model_async() -> None:
    ...
    decorate = middleware_factory(handle_response_model_async=handle_response_model_async)(async_fn)
    result = await decorate()
...
def test_middleware_factory_structured_stream() -> None:
    ...
    my_structured_stream = BaseStructuredStream(stream=base_stream, response_model=Foo, fields_from_call_args={})
    ...
    decorate = middleware_factory(handle_structured_stream=handle_structured_stream)(sync_fn)
...
async def test_middleware_factory_structured_stream_async() -> None:
    ...
    decorate = middleware_factory(handle_structured_stream_async=handle_structured_stream_async)(async_fn)
...
def test_middleware_factory_handle_error_sync() -> None:
    ...
    with pytest.raises(ValueError, match="An error occurred"):
        middleware_factory(handle_error=handle_error)(sync_fn)()
...
async def test_middleware_factory_handle_error_async() -> None:
    ...
    with pytest.raises(ValueError, match="An error occurred"):
        await middleware_factory(handle_error_async=handle_error_async)(async_fn)()
...
@pytest.mark.asyncio
async def test_middleware_factory_handle_error_async_in_handle_call_response_async() -> None:
    class MyCallResponse(BaseCallResponse):
        @property
        def content(self) -> str:
            return "content"  # pragma: no cover
...
def test_middleware_factory_handle_error_sync_in_custom_context_manager() -> None:
    ...
    decorate = middleware_factory(custom_context_manager=custom_context_manager)(sync_fn)
    with pytest.raises(RuntimeError, match="Error in custom_context_manager"):
        decorate()


@pytest.mark.asyncio
async def test_middleware_factory_handle_error_async_in_custom_context_manager() -> None:
    async def async_fn():
        return "result"  # pragma: no cover
    ...
    decorate = middleware_factory(custom_context_manager=custom_context_manager)(async_fn)
    with pytest.raises(RuntimeError, match="Error in custom_context_manager"):
        await decorate()
...
def test_middleware_factory_structured_stream_with_error() -> None:
    ...
    def sync_fn() -> BaseStructuredStream:
        ...
        return MyStructuredStream(stream=base_stream, response_model=Foo, fields_from_call_args={})
...
@pytest.mark.asyncio
async def test_middleware_factory_structured_stream_error_handler_exception_async() -> None:
