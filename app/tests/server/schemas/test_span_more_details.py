...
class TestMessageParam:
    ...
    def test_message_param_creation(self):
        ...
        content = [
            ...
            _ImagePart(type="image", media_type="image/jpeg", image="data", detail=None)
        ]
    ...
    def test_message_param_mixed_content(self):
        ...
        content = [
            ...
            _ImagePart(type="image", media_type="image/png", image="img_data", detail="high"),
...
class TestConvertGeminiMessages:
    ...
    def test_convert_gemini_assistant_response(self):
        ...
        result = convert_gemini_messages(messages)
        assert len(result) == 1  # Only assistant message (no user message for non-gen_ai format)
        assert result[0].role == "assistant"
        assert len(result[0].content) == 0  # Assistant message is empty for unrecognized format
    ...
    def test_convert_gemini_invalid_json(self):
        ...
        result = convert_gemini_messages(messages)
        assert len(result) == 2  # User message + assistant message
        assert result[0].role == "user"  # Invalid JSON still creates user message with content as text
        assert len(result[0].content) == 1
        assert result[0].content[0].type == "text"
...
class TestConvertOpenAIMessages:
    ...
    def test_convert_openai_mixed_content_and_tool_calls(self):
        ...
        result = convert_openai_messages(messages)
        assert len(result) == 1  # Only assistant message
        assert len(result[0].content) == 1  # Only tool call (content handling is different)
        assert result[0].content[0].type == "tool_call"
...
class TestFetchWithMemoryCache:
    ...
    async def test_fetch_with_memory_cache_error(self):
        ...
        with patch('httpx.AsyncClient') as mock_client:
            mock_instance = AsyncMock()
            mock_instance.get.side_effect = httpx.RequestError("Network error", request=Mock())
            mock_client.return_value.__aenter__.return_value = mock_instance
            mock_client.return_value.__aexit__.return_value = None

            # The function doesn't handle httpx errors, so this will raise
            try:
                await fetch_with_memory_cache("http://example.com/api")
...
class TestCalculateOpenrouterCost:
    ...
    @pytest.mark.asyncio
    @patch('lilypad.server.schemas.span_more_details.fetch_with_memory_cache', new_callable=AsyncMock)
...
class TestConvertMirascopeMessages:
    ...
    def test_convert_mirascope_messages_basic(self):
        ...
        messages = [
            {"role": "user", "content": "Hello Mirascope"}
        ]
...
class TestEdgeCasesAndIntegration:
        ...
        mirascope_messages = [
            ...
            },
            {
                ...
                "content": [
                    ...
                    {"type": "image", "media_type": "image/jpeg", "image": "abc123", "detail": "high"}
                ]
            },
            {
                "role": "assistant",
                "content": [
                    ...
                    {"type": "tool_call", "name": "analyze_image", "args": {"image_data": "abc123"}}
                ]
            }
        ]
    ...
    def test_message_param_validation_edge_cases(self):
        ...
        message = MessageParam(
            role="assistant",
            content=[_TextPart(type="text", text="Single response")]
        )
