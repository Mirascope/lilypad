...
def test_image_part():
    ...
    part = _ImagePart(type="image", media_type="image/jpeg", image="data", detail="high")
...
def test_convert_gemini_messages_basic():
    ...
    messages = [{
        "name": "gen_ai.user.message",
        "attributes": {"content": "invalid json"}
    }]
    result = convert_gemini_messages(messages)
    assert len(result) == 2
    assert result[0].role == "user"
...
def test_convert_openai_messages_basic():
    ...
    assert len(result) == 1
    assert result[0].role == "assistant"  # No user/system messages were found in gen_ai format
...
async def test_calculate_openrouter_cost():
    ...
    # Success case
    with patch('lilypad.server.schemas.span_more_details.fetch_with_memory_cache', new_callable=AsyncMock) as mock_fetch:
