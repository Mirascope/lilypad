...
async def test_oauth_callback_success(
    ...
    # Mock handle_user
    with patch(
        f"lilypad.server.api.v0.auth.{callback_module}.handle_user"
    ) as mock_handle_user:
...
@pytest.mark.parametrize(
    "provider,callback_module,error_scenario,expected_error",
    [
        # No code provided
...
async def test_oauth_token_exchange_errors(
    ...
    mock_client_instance.post.side_effect = Exception(
        f"HTTP {error_code}: {error_message}"
    )
