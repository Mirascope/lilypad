...
def test_update_user_consent_privacy_policy_only(client: TestClient):
    ...
    assert updated_consent["privacy_policy_version"] == "2025-05-01"
    new_privacy_accepted = datetime.fromisoformat(updated_consent["privacy_policy_accepted_at"].replace('Z', '+00:00'))
    assert before_update <= new_privacy_accepted <= after_update
...
def test_update_user_consent_tos_only(client: TestClient):
    ...
    assert updated_consent["tos_version"] == "2025-05-01"
    new_tos_accepted = datetime.fromisoformat(updated_consent["tos_accepted_at"].replace('Z', '+00:00'))
    assert before_update <= new_tos_accepted <= after_update
...
def test_post_user_consent_with_preset_timestamps(client: TestClient):
    ...
    before_request = datetime.now(timezone.utc)
    response = client.post("/user-consents", json=consent_data, headers=auth_headers)
    after_request = datetime.now(timezone.utc)
