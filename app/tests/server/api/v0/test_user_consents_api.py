...
def test_post_user_consent_success(client: TestClient):
    ...
    consent_data = {"privacy_policy_version": "2025-04-04", "tos_version": "2025-04-04"}
...
def test_post_user_consent_sets_timestamps(client: TestClient):
    ...
    privacy_accepted = datetime.fromisoformat(
        created_consent["privacy_policy_accepted_at"].replace("Z", "+00:00")
    )
    tos_accepted = datetime.fromisoformat(
        created_consent["tos_accepted_at"].replace("Z", "+00:00")
    )
...
def test_update_user_consent_success(client: TestClient, session: Session):
    ...
    # Update the consent
    update_data = {"privacy_policy_version": "2025-05-01", "tos_version": "2025-05-01"}
...
def test_update_user_consent_privacy_policy_only(client: TestClient):
    ...
    update_data = {"privacy_policy_version": "2025-05-01"}
    ...
    mock_updated_consent = UserConsentTable(
        ...
        tos_accepted_at=datetime.fromisoformat(
            original_tos_accepted.replace("Z", "+00:00")
        ),
        user_uuid=uuid4(),
    )

    with patch(
        "lilypad.server.services.user_consents.UserConsentService.update_record_by_uuid",
        return_value=mock_updated_consent,
    ):
        response = client.patch(f"/user-consents/{consent_uuid}", json=update_data)
        ...
        assert (
            updated_consent["privacy_policy_accepted_at"] != original_privacy_accepted
        )
...
def test_update_user_consent_tos_only(client: TestClient):
    ...
    # Update only ToS
    update_data = {"tos_version": "2025-05-01"}

    # Mock the UserConsentService.update_record_by_uuid method to return a proper response

    mock_updated_consent = UserConsentTable(
        privacy_policy_version="2025-04-04",
        privacy_policy_accepted_at=datetime.fromisoformat(
            original_privacy_accepted.replace("Z", "+00:00")
        ),
...
def test_update_user_consent_null_versions(client: TestClient):
    ...
    update_data = {"privacy_policy_version": None, "tos_version": None}
