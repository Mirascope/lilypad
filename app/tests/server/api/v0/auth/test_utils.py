...
def test_handle_user_existing_user_with_organization():
    ...
    with (
        ...
        patch(
            "lilypad.server.api.v0.auth.utils.OrganizationService"
        ) as mock_org_service_cls,
        patch("lilypad.server.api.v0.auth.utils.is_lilypad_cloud") as mock_is_cloud,
        patch(
            "lilypad.server.api.v0.auth.utils.BillingService"
        ...
        mock_user_public.model_copy.assert_called_once_with(
            update={"access_token": "mock_token"}
        )
...
def test_handle_user_existing_user_self_hosted():
    ...
    with (
        ...
        patch("lilypad.server.api.v0.auth.utils.is_lilypad_cloud") as mock_is_cloud,
        patch(
            "lilypad.server.api.v0.auth.utils.get_organization_license"
        ) as mock_get_license,
        patch("lilypad.server.api.v0.auth.utils.create_jwt_token") as mock_create_token,
    ):
        # Setup mocks
...
def test_handle_user_new_user():
    ...
    with patch(
        "lilypad.server.api.v0.auth.utils.create_new_user"
    ) as mock_create_new_user:
        mock_new_user = Mock()
...
def test_create_new_user_success():
    ...
    with (
        patch("lilypad.server.api.v0.auth.utils.UserTable") as mock_user_table_cls,
        patch("lilypad.server.api.v0.auth.utils.UserPublic") as mock_user_public_cls,
        patch("lilypad.server.api.v0.auth.utils.create_jwt_token") as mock_create_token,
    ):
        # Setup mock user
