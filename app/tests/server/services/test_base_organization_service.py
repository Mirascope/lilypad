...
@pytest.mark.parametrize("use_correct_org,should_succeed", [
...
def test_find_record_by_uuid_with_organization_filter(
    ...
    if use_correct_org:
        ...
        assert found_project.organization_uuid == org_service.user.active_organization_uuid
        
        # Test with explicit organization_uuid filter
        found_project_explicit = org_service.find_record_by_uuid(
            test_project.uuid, organization_uuid=org_service.user.active_organization_uuid
        )
        ...
        with pytest.raises(Exception):  # Should raise HTTPException from base class
            org_service.find_record_by_uuid(test_project.uuid, organization_uuid=wrong_org_uuid)


# ===== Parameterized Tests for find_all_records with Organization Filter =====

@pytest.mark.parametrize("org_filter,additional_filter,expected_min_count", [
...
def test_find_all_records_with_organization_filter(
    org_service, test_project: ProjectTable, org_filter, additional_filter, expected_min_count
):
...
@pytest.mark.parametrize("explicit_org,project_name", [
...
def test_organization_filters_pop_behavior(org_service):
    ...
    assert another_project.organization_uuid == org_service.user.active_organization_uuid


# ===== Inheritance Tests =====

@pytest.mark.parametrize("method_name,has_method", [
    ("find_record", True),
