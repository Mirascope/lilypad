...
def test_comment(comment_service: CommentService, db_session: Session) -> CommentTable:
    ...
    span_uuid = uuid4()
    comment_data = CommentCreate(span_uuid=span_uuid, text="Test comment content")
    comment = comment_service.create_record(
        comment_data, user_uuid=comment_service.user.uuid
    )
    db_session.commit()
...
def test_find_by_spans_with_comments(
    comment_service: CommentService, test_comment: CommentTable, db_session: Session
):
    ...
    for comment in found_comments:
        ...
        assert (
            comment.organization_uuid == comment_service.user.active_organization_uuid
        )
...
def test_find_by_spans_multiple_comments_ordered(
    comment_service: CommentService, db_session: Session
):
    """Test that comments are returned in created_at order (ascending)."""
    span_uuid = uuid4()

    # Create multiple comments for the same span
    comment1_data = CommentCreate(span_uuid=span_uuid, text="First comment")
    comment1 = comment_service.create_record(
        comment1_data, user_uuid=comment_service.user.uuid
    )
    db_session.flush()

    comment2_data = CommentCreate(span_uuid=span_uuid, text="Second comment")
    comment2 = comment_service.create_record(
        comment2_data, user_uuid=comment_service.user.uuid
    )
    db_session.flush()

    comment3_data = CommentCreate(span_uuid=span_uuid, text="Third comment")
    comment3 = comment_service.create_record(
        comment3_data, user_uuid=comment_service.user.uuid
    )
    db_session.commit()
...
def test_find_by_spans_organization_filter(
    ...
    comment_data = CommentCreate(span_uuid=span_uuid, text="User organization comment")
    user_comment = comment_service.create_record(
        comment_data, user_uuid=comment_service.user.uuid
    )
    db_session.commit()
    ...
    assert (
        found_comments[0].organization_uuid
        == comment_service.user.active_organization_uuid
    )
...
def test_create_record_adds_organization(
    ...
    comment_data = CommentCreate(
        span_uuid=span_uuid, text="Test comment for organization"
    )
    created_comment = comment_service.create_record(
        comment_data, user_uuid=comment_service.user.uuid
    )

    # Should have organization_uuid set from the user
    assert (
        created_comment.organization_uuid
        == comment_service.user.active_organization_uuid
    )
...
def test_find_by_spans_with_different_spans(
    ...
    comment1_data = CommentCreate(span_uuid=span1_uuid, text="Comment for span 1")
    comment1 = comment_service.create_record(
        comment1_data, user_uuid=comment_service.user.uuid
    )

    comment2_data = CommentCreate(span_uuid=span2_uuid, text="Comment for span 2")
    comment2 = comment_service.create_record(
        comment2_data, user_uuid=comment_service.user.uuid
    )
    db_session.commit()
