...
class TestStripeQueueProcessor:
    ...
    def processor(self):
        ...
        with patch(
            "lilypad.server.services.stripe_queue_processor.get_settings"
        ) as mock_settings:
    ...
    async def test_initialize_success(self, processor):
        ...
        with (
            ...
            patch(
                "lilypad.server.services.stripe_queue_processor.AIOKafkaProducer"
    ...
    async def test_flush_pending_batches_with_failures(self, processor):
        ...
        with (
            patch("asyncio.gather", return_value=results),
            patch(
    ...
    def test_update_stripe_sync(self, processor):
        ...
        with (
            patch(
                "lilypad.server.services.stripe_queue_processor.get_session"
            ) as mock_get_session,
            patch(
                "lilypad.server.services.stripe_queue_processor.BillingService"
            ...
            result = processor._update_stripe_sync(
                user_uuid, org_uuid, quantity, idempotency_key, trace_count
            )
    ...
    async def test_process_meter_event_batch_overflow(self, processor):
        ...
        assert (
            len(
                [
                    k
                    for k in processor.failed_batches
                    if k.startswith(f"{org_uuid}_overflow_")
                ]
            )
            == 1
        )
    ...
    async def test_flush_pending_batches_with_age_based_batches(self, processor):
        ...
        with patch.object(
            processor, "_send_batch_with_rate_limit", return_value=True
        ) as mock_send:
            await processor._flush_pending_batches()
    ...
    async def test_flush_pending_batches_partial_failure(self, processor):
        ...
        with patch.object(
            processor, "_send_batch_with_rate_limit", side_effect=mock_send_side_effect
        ):
    ...
    async def test_send_batch_with_rate_limit_failure(self, processor):
        ...
        with patch.object(
            processor, "_update_stripe_sync", side_effect=Exception("Stripe error")
        ):
            result = await processor._send_batch_with_rate_limit(batch_key, batch)
    ...
    async def test_retry_loop_with_backoff(self, processor):
        ...
        batch.last_attempt = datetime.now(timezone.utc) - timedelta(
            minutes=5
        )  # 5 minutes ago
