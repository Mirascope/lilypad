...
@pytest.mark.parametrize("billing_state,price_ids,expected_tier", [
    # No billing record
    (None, {}, Tier.FREE),
...
def test_get_tier_from_billing(mock_settings, billing_service, db_session, billing_state, price_ids, expected_tier):
...
@pytest.mark.parametrize("existing_customer_id,should_create_new", [
...
def test_delete_customer_and_billing_success(
    ...
    mock_stripe.Customer.delete.assert_called_once_with(mock_billing_record.stripe_customer_id)
...
def test_get_customer_stripe_error(
    ...
    # Mock Stripe error
    mock_stripe.Customer.retrieve.side_effect = stripe.error.StripeError(
        "API error"
    )
...
@pytest.mark.parametrize("event_type,should_update_billing", [
...
@pytest.mark.parametrize("message_type,expected_method", [
...
async def test_report_span_usage_with_fallback_kafka_success(
    ...
    await billing_service.report_span_usage_with_fallback(
        sample_organization.uuid,
        100,
        stripe_kafka_service=mock_kafka_service
    )
...
def test_delete_customer_generic_stripe_error(
    ...
    mock_stripe.Customer.delete.side_effect = stripe.error.StripeError(
        "Generic error"
    )
...
def test_update_customer_generic_stripe_error(
    ...
    # Mock Stripe error
    mock_stripe.Customer.modify.side_effect = stripe.error.StripeError(
        "Generic error"
    )
...
def test_report_span_usage_success(
    ...
    with patch("lilypad.server.services.billing.settings") as mock_settings:
        ...
        assert call_args["event_name"] == "spans"  # Note: it's "spans" not "span_usage" in the code
