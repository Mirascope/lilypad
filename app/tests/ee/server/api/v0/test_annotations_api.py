...
from lilypad.ee.server.schemas.annotations import AnnotationCreate, AnnotationUpdate
from lilypad.server.models import FunctionTable, ProjectTable, SpanTable, UserTable
...
def test_span(session: Session, test_project: ProjectTable) -> SpanTable:
    ...
    span = SpanTable(
        ...
        data={"attributes": {"lilypad.type": "llm", "lilypad.llm.output": "test output"}},
    )
    session.add(span)
    session.commit()
...
@pytest.fixture
def test_annotation(
    session: Session, test_project: ProjectTable, test_span: SpanTable, test_user: UserTable
) -> AnnotationTable:
    ...
    annotation = AnnotationTable(
        ...
        data={"output": {"idealOutput": "test", "reasoning": "test", "exact": True, "label": "success"}},
    )
...
class TestCreateAnnotations:
    ...
    def test_create_annotations_simple(
        self, mock_project_service_cls, mock_annotation_service_cls, client: TestClient, test_project: ProjectTable
    ):
        ...
        with patch("lilypad.ee.server.api.v0.annotations_api.SpanMoreDetails.from_span") as mock_span_details:
    ...
    def test_create_annotations_with_duplicates(
        ...
        # Mock services
        mock_annotation_service = Mock()
        mock_annotation_service.check_bulk_duplicates.return_value = [uuid4()]  # Return duplicate
        mock_annotation_service_cls.return_value = mock_annotation_service
...
class TestUpdateAnnotation:
    ...
    def test_update_annotation(
        self, mock_annotation_service_cls, client: TestClient, test_project: ProjectTable
    ):
...
class TestGetAnnotations:
    ...
    def test_get_annotations_by_project(
        ...
        with patch("lilypad.ee.server.api.v0.annotations_api.SpanMoreDetails.from_span") as mock_span_details:
            ...
            assert response.status_code == 200
            mock_annotation_service.find_records_by_project_uuid.assert_called_once_with(test_project.uuid)
...
class TestAnnotationMetrics:
    ...
    def test_get_annotation_metrics_by_function(
        ...
        # Mock service
        mock_annotation_service = Mock()
        mock_annotation_service.find_metrics_by_function_uuid.return_value = (5, 10)  # (success, total)
        mock_annotation_service_cls.return_value = mock_annotation_service
...
class TestGenerateAnnotation:
    ...
    def test_generate_annotation_with_span_string_output(
        ...
        # Mock services
        mock_annotation_service = Mock()
        mock_annotation_service.find_record_by_span_uuid.return_value = None  # No existing annotation
        mock_annotation_service_cls.return_value = mock_annotation_service
