...
from lilypad.ee.server.schemas.user_organizations import UserOrganizationUpdate
from lilypad.server.models import OrganizationInviteTable, OrganizationTable, UserTable


@pytest.fixture
def test_user_organization(session: Session, test_user: UserTable) -> UserOrganizationTable:
    """Get the test user organization created in the conftest."""
    # The user organization is already created in the conftest
    user_org = session.query(UserOrganizationTable).filter_by(user_uuid=test_user.uuid).first()
    if not user_org:
...
def test_create_user_organization_success_self_hosted(
    mock_is_cloud, client: TestClient, test_organization_invite: OrganizationInviteTable, session: Session
):
    ...
    with patch("lilypad.ee.server.api.v0.user_organizations_api.LicenseValidator") as mock_validator:
...
def test_create_user_organization_success_cloud(
    ...
    with patch("lilypad.ee.server.api.v0.user_organizations_api.BillingService") as mock_billing:
...
def test_create_user_organization_user_limit_exceeded(
    mock_is_cloud, client: TestClient, test_organization_invite: OrganizationInviteTable, test_user_organization: UserOrganizationTable
):
    ...
    with patch("lilypad.ee.server.api.v0.user_organizations_api.LicenseValidator") as mock_validator:
        ...
        with patch("lilypad.ee.server.api.v0.user_organizations_api.cloud_features") as mock_features:
            mock_feature = Mock()
...
def test_create_user_organization_invite_delete_failed(
    ...
    with patch("lilypad.ee.server.api.v0.user_organizations_api.is_lilypad_cloud") as mock_is_cloud:
        mock_is_cloud.return_value = False
        ...
        with patch("lilypad.ee.server.api.v0.user_organizations_api.LicenseValidator") as mock_validator:
            ...
            with patch("lilypad.server.services.organization_invites.OrganizationInviteService.delete_record_by_uuid") as mock_delete:
...
def test_update_user_organization(client: TestClient, test_user_organization: UserOrganizationTable):
    """Test updating a user organization role."""
    update_data = {"role": UserRole.MEMBER.value}
    
    response = client.patch(f"/ee/user-organizations/{test_user_organization.uuid}", json=update_data)
    assert response.status_code == 200
...
def test_delete_user_organization(client: TestClient, test_user_organization: UserOrganizationTable):
...
def test_create_user_organization_without_license_info(
    ...
    with patch("lilypad.ee.server.api.v0.user_organizations_api.is_lilypad_cloud") as mock_is_cloud:
        ...
        with patch("lilypad.ee.server.api.v0.user_organizations_api.LicenseValidator") as mock_validator:
            ...
            response = client.post("/ee/user-organizations", json=token_data)
            assert response.status_code == 200  # Should still work with FREE tier default
...
def test_create_user_organization_create_failed(
    ...
    with patch("lilypad.ee.server.api.v0.user_organizations_api.is_lilypad_cloud") as mock_is_cloud:
        ...
        with patch("lilypad.ee.server.api.v0.user_organizations_api.LicenseValidator") as mock_validator:
            ...
            with patch("lilypad.ee.server.services.user_organizations.UserOrganizationService.create_record") as mock_create:
