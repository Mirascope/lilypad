...
def upgrade() -> None:
    ...
    sa.Enum('RESOLVED', 'PENDING', 'ORPHANED', name='parentstatus').create(op.get_bind())
    with op.batch_alter_table('spans', schema=None) as batch_op:
        batch_op.add_column(sa.Column('parent_status', postgresql.ENUM('RESOLVED', 'PENDING', 'ORPHANED', name='parentstatus', create_type=False), nullable=False, server_default='RESOLVED'))
        batch_op.create_index('idx_spans_parent_status', ['parent_status'], unique=False)
        batch_op.create_index('idx_spans_pending_parent', ['parent_span_id', 'parent_status'], unique=False)
