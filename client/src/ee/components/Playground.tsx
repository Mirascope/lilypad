...
export const Playground = ({
  ...
  return (
    <Form {...methods}>
      <PanelGroup direction="horizontal" className="flex-1 h-full border rounded-lg overflow-hidden"> {/* Added overflow-hidden */}

        <Panel defaultSize={showRightPanel ? 50 : 100} minSize={30} order={1}>
          <div className="flex flex-col h-full p-4 overflow-y-auto"> {/* Make panel scrollable if needed */}
            <form
              id={`playground-form-${version?.uuid ?? Math.random().toString(36).substring(7)}`}
              onSubmit={methods.handleSubmit(onSubmit)}
              className='flex flex-col gap-4 flex-1 h-full' // Ensure form structure takes space
            >
               ...
               <div className='lexical flex-1 min-h-[200px] relative'> {/* Added min-h, adjust as needed */}
                 <Editor
        ...
        {showRightPanel && (
         ...
         <Panel
            defaultSize={showRightPanel ? 50 : 0}
            minSize={showRightPanel ? 25 : 0}
            collapsible={true}
            collapsedSize={0}
            order={2}
