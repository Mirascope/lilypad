...
def upgrade() -> None:
    ...
    sa.Column('dependencies', lilypad.server.models.base_sql_model.JSONTypeDecorator(), nullable=True),
    ...
    sa.ForeignKeyConstraint(['generation_uuid'], ['generations.uuid'], name=op.f('tools_generation_uuid_generations_fkey'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['organization_uuid'], ['organizations.uuid'], name=op.f('tools_organization_uuid_organizations_fkey'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['project_uuid'], ['projects.uuid'], name=op.f('tools_project_uuid_projects_fkey'), ondelete='CASCADE'),
    sa.ForeignKeyConstraint(['span_uuid'], ['spans.uuid'], name=op.f('tools_span_uuid_spans_fkey'), ondelete='CASCADE'),
    ...
    with op.batch_alter_table('tools', schema=None) as batch_op:
        ...
        batch_op.create_index(batch_op.f('tools_organization_uuid_idx'), ['organization_uuid'], unique=False)

    with op.batch_alter_table('generations', schema=None) as batch_op:
        batch_op.add_column(sa.Column('prompt_template', sqlmodel.sql.sqltypes.AutoString(), nullable=True))
        batch_op.add_column(sa.Column('call_params', lilypad.server.models.base_sql_model.JSONTypeDecorator(), nullable=True))
