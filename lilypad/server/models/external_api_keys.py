...
class ExternalAPIKeyBase(SQLModel):
    ...
    service_name: str = Field(nullable=False, description="Name of the external service")
    secret_id: str = Field(
...
class ExternalAPIKeyTable(ExternalAPIKeyBase, SQLModel, table=True):
    ...
    user_id: UUID = Field(foreign_key=f"{USER_TABLE_NAME}.uuid", nullable=False, description="ID of the user")
