...
def test_model_generate(mock_tracer, mock_span):
    decorated = model_generate(mock_tracer)(MockModel.generate)
    ...
    result = decorated(MockModel(), "prompt", gp, lp, sp)
...
def test_model_generate_error(mock_tracer, mock_span):
    ...
    decorated_error = model_generate(mock_tracer)(error_generate)
    gp = FakeGenerationParameters(stop_at="STOP")
    lp = object()
    sp = FakeSamplingParameters()

    with pytest.raises(Exception) as exc_info:
        decorated_error(MockModel(), "prompt", gp, lp, sp)
...
def test_model_generate_stream(mock_tracer, mock_span):
    decorated = model_generate_stream(mock_tracer)(MockModel.stream)
    ...
    chunks = list(decorated(MockModel(), "prompt"))
...
def test_model_generate_stream_error(mock_tracer, mock_span):
    ...
    decorated_error = model_generate_stream(mock_tracer)(error_stream)
    gp = FakeGenerationParameters(stop_at="STOP")

    with pytest.raises(Exception) as exc_info:
        list(decorated_error(MockModel(), "prompt", generation_parameters=gp))
...
async def test_model_generate_async(mock_tracer, mock_span):
    decorated = model_generate_async(mock_tracer)(MockModel.generate_chat)

    gp = FakeGenerationParameters(stop_at="STOP")
    lp = object()
    sp = FakeSamplingParameters()

    result = await decorated(MockModel(), "prompt", gp, lp, sp, None)
...
async def test_model_generate_async_error(mock_tracer, mock_span):
    ...
    decorated_error = model_generate_async(mock_tracer)(error_async)
    gp = FakeGenerationParameters(stop_at="STOP")
    lp = object()
    sp = FakeSamplingParameters()

    with pytest.raises(Exception) as exc_info:
        await decorated_error(MockModel(), "prompt", gp, lp, sp, None)
